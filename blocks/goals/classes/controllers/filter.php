...
class filter {
    ...
    public static function display($id, $categoryid=0, $form=null) {
        ...
        if (empty($id)) {
            ...
            $record = $filter->read();
            $strheading = get_string('editfilter', 'block_goals', format_string($record->get('name')));
        }
    ...
    public static function process($id) {
        ...
        if (!empty($id)) {
            $filter = new models\filter($id);
            $customdata['persistent'] = $filter;
            $customdata['sortorder'] = $filter->get('sortorder');
        }
        $form = new forms\filter($PAGE->url->out(false), $customdata);
        ...
        if ($data = $form->get_data()) {
            ...
            try {
                ...
                if (empty($data->id)) {
                    ...
                    if ($filter->update()) {
                        notification::success("filter $data->name updated");
                    }
                }
                // $filter->reorder();
    ...
    public static function icon($fitler) {
        if (empty($fitler->get('faicon'))) {
            return '';
        } else {
            return '<i class="' . $fitler->get('faicon') . '"></i>' . PHP_EOL;
        }
    }
}