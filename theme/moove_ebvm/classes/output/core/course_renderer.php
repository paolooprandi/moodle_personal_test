...
namespace theme_moove_ebvm\output\core;
...
use theme_moove_ebvm\util\extras;

/**
 * Renderers to align moove_ebvm's course elements to what is expect
 *
 * @package    theme_moove_ebvm
...
class course_renderer extends \core_course_renderer {
    ...
    protected function coursecat_courses(coursecat_helper $chelper, $courses, $totalcount = null) {
        ...
        $theme = \theme_config::load('moove_ebvm');
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course) {
        ...
        // TODO: Frontpage not logged in for courses with guest user hacks
        $showmodalwarningforguestcourse = false;

        // If already logged in as guest user, or is not yet logged in.
        if (isguestuser() || !isloggedin()) {
            // Name of enrolment plugin to target.
            $enrolmentmethod = 'guest';

            // Ensure enrolment method is enabled.
            if (enrol_is_enabled($enrolmentmethod)) {
                // Does enrolment method instance exists for course.
                $enrolmentinstance = $DB->get_record('enrol', ['enrol' => $enrolmentmethod, 'courseid' => $course->id]);
                if ($enrolmentinstance) {
                    // Ensure enrolment instance is enabled. Constant defined in lib/enrollib.php.
                    if ($enrolmentinstance->status == ENROL_INSTANCE_ENABLED) {

                        // User is not logged in and course allows guest access.
                        $showmodalwarningforguestcourse = true;
                    }
                }
            }
        }

        $guestusermodalattributes = [];
        if ($showmodalwarningforguestcourse) {
            $guestusermodalattributes = [
                'data-toggle' => 'modal',
                'data-target' => '#modalguestcourse' . $course->id
            ];
        }
        ...
        $coursenamelink = html_writer::link($courselink, $coursename, array_merge($guestusermodalattributes, array('class' => $course->visible ? '' : 'dimmed')));
        ...
        if (isloggedin()) {
            ...
            $content .= html_writer::link(new moodle_url('/course/view.php', array('id' => $course->id)),
                get_string('access', 'theme_moove_ebvm'), array('class' => 'card-link btn btn-primary'));
        ...
        // TODO: Less hacky for now
        if ($showmodalwarningforguestcourse) {
            $content .= '
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalguestcourse' . $course->id . '"
          style="border-radius: 25px !important; width: 90%; margin-bottom: 1em; align-self: center;" >
            Access <i class="fa fa-chevron-circle-right" style="float: right; padding-top: 3px"></i>
        </button>

        <!-- Modal -->
        <div class="modal fade" id="modalguestcourse' . $course->id . '" tabindex="-1" role="dialog" aria-labelledby="modalguestcourse' . $course->id . 'Title" aria-hidden="true">
                ...
                <div class="modal-content">
                  ...
                  <span aria-hidden="true">×</span>
                    ...
                    <div class="modal-footer">
                        <a class="btn btn-primary" href="' . $courselink->out() . '">Continue to course
                </a>
