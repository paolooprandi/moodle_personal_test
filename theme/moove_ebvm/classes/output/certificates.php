...
class certificates implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $certificates = new \theme_moove_ebvm\util\certificates($USER, $this->courseid);

        $issuedcertificates = $certificates->get_all_certificates();

        $title = get_string('certificatestitle', 'theme_moove_ebvm');
        $subtitle = get_string('subtitleallcertificates', 'theme_moove_ebvm');

        if ($this->courseid) {
            $subtitle = get_string('subtitlecoursecertificates', 'theme_moove_ebvm');
        }
