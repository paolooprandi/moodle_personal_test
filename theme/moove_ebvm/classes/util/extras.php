...
/**
 * Custom moove_ebvm extras functions
 *
...
class extras {
    ...
    public static function get_course_summary_image($course, $courselink) {
        ...
        // Frontpage not logged in guest user hacks
        ...
        foreach ($course->get_course_overviewfiles() as $file) {
            ...
            if ($isimage) {
                $contentimage = \html_writer::link($courselink, \html_writer::empty_tag('img', array(
                    'src' => $url,
                    'alt' => $course->fullname,
                    'class' => 'card-img-top w-100')), $guestusermodalattributes);
                break;
            }
        }

        if (empty($contentimage)) {
            $url = $CFG->wwwroot . "/theme/moove_ebvm/pix/default_course.jpg";
    ...
    public static function get_mypublic_headerbuttons($context, $user) {
        ...
        if (!empty($CFG->messaging) && $USER->id != $user->id && has_capability('moodle/site:sendmessage', $context)) {
            ...
            $headerbuttons = [
                ...
                [
                    'title' => get_string($contacttitle, 'theme_moove_ebvm'),
