...
/**
 * Frontpage layout for the moove_ebvm theme.
 *
...
$themesettings = new \theme_moove_ebvm\util\theme_settings();
...
if (isloggedin() && !isguestuser()) {
    ...
    theme_moove_ebvm_extend_flat_navigation($PAGE->flatnav);

    $templatecontext['flatnavigation'] = $PAGE->flatnav;

    $templatecontext = array_merge($templatecontext, $themesettings->footer_items(), $themesettings->slideshow());

    echo $OUTPUT->render_from_template('theme_moove_ebvm/frontpage', $templatecontext);
} else {
    $sliderfrontpage = false;
    if ((theme_moove_ebvm_get_setting('sliderenabled', true) == true) && (theme_moove_ebvm_get_setting('sliderfrontpage', true) == true)) {
        $sliderfrontpage = true;
        $extraclasses[] = 'slideshow';
    }

    $numbersfrontpage = false;
    if (theme_moove_ebvm_get_setting('numbersfrontpage', true) == true) {
        $numbersfrontpage = true;
    }

    $sponsorsfrontpage = false;
    if (theme_moove_ebvm_get_setting('sponsorsfrontpage', true) == true) {
        $sponsorsfrontpage = true;
    }

    $clientsfrontpage = false;
    if (theme_moove_ebvm_get_setting('clientsfrontpage', true) == true) {
        $clientsfrontpage = true;
    }

    $bannerheading = '';
    if (!empty($PAGE->theme->settings->bannerheading)) {
        $bannerheading = theme_moove_ebvm_get_setting('bannerheading', true);
    }

    $bannercontent = '';
    if (!empty($PAGE->theme->settings->bannercontent)) {
        $bannercontent = theme_moove_ebvm_get_setting('bannercontent', true);
    }

    $shoulddisplaymarketing = false;
    if (theme_moove_ebvm_get_setting('displaymarketingbox', true) == true) {
        $shoulddisplaymarketing = true;
    }

    $disablefrontpageloginbox = false;
    if (theme_moove_ebvm_get_setting('disablefrontpageloginbox', true) == true) {
    ...
    echo $OUTPUT->render_from_template('theme_moove_ebvm/frontpage_guest', $templatecontext);
}
