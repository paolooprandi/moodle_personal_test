 ...
 * @return string
 */
function theme_moove_ebvm_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_moove_ebvm_set_headerimg($theme);

    $scss .= theme_moove_ebvm_set_topfooterimg($theme);

    $scss .= theme_moove_ebvm_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_moove_ebvm_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_moove_ebvm_set_topfooterimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_moove_ebvm_set_loginbgimg($theme) {
    global $OUTPUT;
    ...
    $headercss = "#page-login-index.moove_ebvm-login #page-wrapper #page {}";
 ...
 * @return string
 */
function theme_moove_ebvm_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_moove_ebvm', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_moove_ebvm and not theme_boost (see the line above).
    ...
    // moove_ebvm scss.
    $moove_ebvmvariables = file_get_contents($CFG->dirroot . '/theme/moove_ebvm/scss/moove_ebvm/_variables.scss');
    $moove_ebvm = file_get_contents($CFG->dirroot . '/theme/moove_ebvm/scss/moove_ebvm.scss');

    // Combine them together.
    return $moove_ebvmvariables . "\n" . $scss . "\n" . $moove_ebvm;
}
 ...
 * @return string
 */
function theme_moove_ebvm_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_moove_ebvm_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('moove_ebvm');
 ...
 * @return string
 */
function theme_moove_ebvm_get_setting($setting, $format = false) {
    $theme = theme_config::load('moove_ebvm');
...
/**
 * Extend the moove_ebvm navigation
 *
 * @param flat_navigation $flatnav
 */
function theme_moove_ebvm_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_moove_ebvm_add_certificatesmenuitem($flatnav);

    theme_moove_ebvm_delete_menuitems($flatnav);

    // theme_moove_ebvm_add_coursesections_to_navigation($flatnav);
}
 ...
 *
 */
function theme_moove_ebvm_add_certificatesmenuitem(\flat_navigation $flatnav) {
    global $COURSE;

    try {
        if (!theme_moove_ebvm_has_certificates_plugin()) {
            return;
        }

        $actionurl = new \moodle_url('/theme/moove_ebvm/certificates.php');

        // Course page.
        if ($COURSE->id > 1) {
            $parentitem = $flatnav->find('competencies', \navigation_node::TYPE_SETTING);

            $actionurl = new \moodle_url('/theme/moove_ebvm/certificates.php', ['id' => $COURSE->id]);
        }
	...
	if (is_object($parentitem)) {
        if (!is_null($parentitem->parent)) {
            $certificatesitemoptions = [
                'action' => $actionurl,
                'text' => get_string('certificates', 'theme_moove_ebvm'),
                'shorttext' => get_string('certificates', 'theme_moove_ebvm'),
 ...
 * @param flat_navigation $flatnav
 */
function theme_moove_ebvm_delete_menuitems(\flat_navigation $flatnav) {
 ...
 * @param flat_navigation $flatnav
 */
function theme_moove_ebvm_add_coursesections_to_navigation(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_moove_ebvm'),
            'shorttext' => get_string('coursesections', 'theme_moove_ebvm'),
 ...
 * @return bool
 */
function theme_moove_ebvm_has_certificates_plugin() {
